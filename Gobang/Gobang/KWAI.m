...
//    NSLog(@"is Step emergent");
    ...
    if (![self checkPoint:check]) {
        ...
        KWPoint *test = [[KWPoint alloc] initPointWith:check.x y:check.y]; // 开始进行寻找的点
        KWPoint *testR = [[KWPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的左侧的点
        KWPoint *testL = [[KWPoint alloc] initPointWith:check.x y:check.y]; // 遍历点的右侧的点
        
        testR = [[KWPoint alloc] initPointWith:check.x + 1 y:check.y]; // 开始遍历点的正右侧
        testL = [[KWPoint alloc] initPointWith:check.x - 1 y:check.y]; // 开始遍历的点的正左侧
        ...
        testR = [[KWPoint alloc] initPointWith:check.x y:check.y + 1]; // 开始遍历点的正下方
        testL = [[KWPoint alloc] initPointWith:check.x y:check.y - 1]; // 开始遍历的点的正上方
